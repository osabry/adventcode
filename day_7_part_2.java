...
public class day_7_part_2 {
    ...
    private static long check(long result, String[] myNums) {
        ...
        for (int i = 0; i < size_of_operation; i++) {
            ...
            // }
            // System.out.println(equeation + ((tmp==result)?" = ":" != ") + tmp + " current
            // sum = " + totla_sum) ;
            ...
            if (result == calculate(numbers, operations)) {
                ...
                // equeation += " " + tttt.charAt(k-1) + " " + numbers[k];
                // }
                // System.out.println(equeation + " = " + result + " current sum = " +
                // totla_sum) ;
                return result;
            }
            else { 
                // reshuffle 
                // System.out.println(result + " : " + numString + "    with     " + operations +  " need to reshuffle ----- ");
                int len_new = operations.length();
                for(int j = 0 ; j < len_new; j++ ) {
                    String new_operation = operations.substring(0, j) + "|" + operations.substring(j+1, len_new) ; 
                    System.out.println( result + " : " + numString + "         reshuffle step " + j + "      " + new_operation );
                    if (result == calculate(numbers, new_operation)) {
                         System.out.println( "          -------------------- > found new match for " + new_operation );
                        return result;
                    }
                } 
        ...
        private static long calculate(Long[] numbers, String operations) {
            ...
            for (int i = 1; i < numbers.length; i++) {
                ...
                if (operations.charAt(i - 1) == '0') {
                    result += numbers[i];
                } else if (operations.charAt(i - 1) == '1') {
                    result *= numbers[i];
                } else {
                    System.out.print("                 found | operation , " + result );
                    String ss = "" + result + "" + numbers[i];
...
// day 7 part 2    ------------total sum of calibrated : 348360680516005    uused the day_7_part_2_2 
